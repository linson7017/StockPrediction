...
from debt_dataset import GetDataset
...
def run(stock: str, model_type: str, stationary=True):
    df = Analysis.get_data(stock,['CompanyCode', 'Date'])
    dataset = GetDataset(df)
    dataset.get_dataset(scale="", stationary=stationary)
    ...
    hist = clf.train([x_train, y_train], params=params)
    torch.save(clf.model,"model/model_{}_{}_{}.pth".format(model_type,hist[-1],datetime.now().strftime('%Y-%m-%d')))
